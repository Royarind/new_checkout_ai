(val) => {
    async function patternMatch(args) {
        const variantValue = typeof args === 'string' ? args : args.variantValue;
        const containerSelector = typeof args === 'object' ? args.containerSelector : null;
        ...
        if (containerSelector) {
            ...
            if (container) {
                root = container;
                console.log(`Pattern Match restricted to container: ${containerSelector}`);
            } else {
        ...
        const match = (text) => {
            ...
            // For short strings (like "M", "L", "XL"), enforce exact match only
        ...
        const searchPatterns = [
            ...
            // PRIORITY 1: Labels with radio/checkbox inputs (highest priority for color/size selection)
            {
                selector: 'label[for]', action: 'click',
                extraCheck: (el) => {
            ...
            // PRIORITY 2: Standalone radio/checkbox inputs
            {
            ...
            // Universal element traversal (labels, spans, divs - NO images to prevent false positives)
            {
                selector: 'label, span, div, li, td, a', action: 'click',  // Removed 'img' - images handled by specialized patterns only
                extraCheck: (el) => {
                    ...
                    // If no better element found, use original element ONLY if it is interactive
                    // For short values ("M", "L"), be extra strict

                    // CRITICAL: Skip navigation/header elements
                    let isInNavigation = false;
                    let checkParent = el;
                    let navDepth = 0;
                    while (checkParent && navDepth < 10) {
                    ...
                    if (isInNavigation) return false;
                    ...
                    const isInteractive = el.onclick || el.getAttribute('onclick') ||
                        el.style.cursor === 'pointer' ||
                        (el.classList && el.classList.contains('clickable')) ||
                        (el.classList && el.classList.contains('selectable')) ||
                        el.hasAttribute('role') ||
                        el.tagName === 'BUTTON' || (el.tagName === 'A' && rect.top >= 150) || el.tagName === 'INPUT';

                    if (isInteractive) return true;

                    // If strict mode (short string) and not interactive, reject it
                    if (normalizedVal.length < 3) return false;

                    // Otherwise, leniently accept it (legacy behavior for longer text)
                    return true;
                }
            },
            ...
            // Buttons
            {
                ...
                extraCheck: (el) => {
                    if (match(el.textContent) || match(el.getAttribute('aria-label')) ||
                        match(el.value) || match(el.getAttribute('title'))) return true;

                    // Check child images (common for color swatches)
                    const img = el.querySelector('img');
                    if (img && (match(img.alt) || match(img.getAttribute('title')))) return true;
        ...
        for (const pattern of searchPatterns) {
            ...
            for (const el of elements) {
                ...
                // Skip table elements unless they have explicit interaction roles
                if (['TD', 'TH', 'TR', 'TABLE', 'TBODY', 'THEAD'].includes(el.tagName)) {
                    const isExplicitlyInteractive = el.hasAttribute('onclick') ||
                        el.getAttribute('role') === 'button' ||
                        el.classList.contains('clickable') ||
                        el.classList.contains('selectable');
                    if (!isExplicitlyInteractive) continue;
                }
                ...
                if (isMatch) {
                    ...
                    const actionData = {
                        ...
                        element: {
                            ...
                            outerHTML: el.outerHTML ? el.outerHTML.substring(0, 100) : ''
                        }
                    };
