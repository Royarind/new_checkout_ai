        ...
        result = await page.evaluate("""
            () => {
                ...
                const cookieSelectors = [
                    ...
                    '#cookie-banner button', '#cookie-consent button',
                    '[data-cookie] button', '#onetrust-accept-btn-handler',
                    '#truste-consent-button', '.gdpr-accept', '.cookie-accept'
                ];
                ...
                // Prioritize X close buttons first, then accept buttons
                const allSelectors = [
                    ...closeSelectors,
                    ...
                    try {
                        ...
                        for (const el of elements) {
                            ...
                            if (el.offsetParent !== null && typeof el.click === 'function') {
                                ...
                                // SKIP buttons that open things or are primary actions
                                const skipWords = [
                                    'add to cart', 'add to bag', 'buy now', 'checkout', 'submit', 
                                    'subscribe', 'sign up', 'shop', 'view', 'learn more', 'see more',
                                    'continue shopping', 'keep shopping', 'start shopping'
                                ];
                                const shouldSkip = skipWords.some(word => 
                                    text.includes(word) || ariaLabel.includes(word)
                                );
                                
                                // ONLY click if it's clearly a close/dismiss button
                                const isCloseButton = 
                                    text === '×' || text === '✕' || text === '✖' || text === '⨉' ||
