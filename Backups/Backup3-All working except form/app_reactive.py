...
async def run_reactive_checkout(payload, user_prompt_callback=None):
    """Run checkout with reactive agent
    user_prompt_callback: Optional callback for prompting user (e.g., password)
    """
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            for i, task in enumerate(tasks):
                ...
                print("⏳ Waiting for page to stabilize (5 seconds)...")
                await asyncio.sleep(5)
                
                # Aggressively clear all popups and modals
                from shared.popup_dismisser import dismiss_popups
                from shared.page_analyzer import analyze_page_content
                
                print("🧹 Aggressively clearing all popups, cookies, and modals...")
                for attempt in range(12):
                    ...
                    if analysis['hasBlockingOverlay']:
                        print(f"    Found {len(analysis['overlayInfo'])} overlays, clearing...")
                        # Click outside to dismiss side modals
                        try:
                        ...
                        if attempt >= 3:
                            print("✅ Viewport clear - ready for variant selection")
                            break
                    
                    await asyncio.sleep(2)
                
                print("⏳ Final wait before variant selection (3 seconds)...")
                await asyncio.sleep(3)
                
                # Select variants after viewport is clear
