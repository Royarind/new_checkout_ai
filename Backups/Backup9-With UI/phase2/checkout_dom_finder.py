    ...
    for attempt in range(max_retries):
        try:
            ...
            try:
                ...
                # Random delay between 100-150ms per character for human-like typing
                import random
...
async def batch_fill_fields(page, field_mappings):
    ...
    try:
        ...
        for i, mapping in enumerate(field_mappings):
            ...
            if result['success']:
                filled_count += 1
                # Random delay between 0.5-1.5 seconds between fields
                import random
    ...
    for attempt in range(max_retries):
        try:
            ...
            for select, select_info in matched_selects:
                ...
                values_to_try = [option_value, option_value.upper(), option_value.lower(), option_value.title()]
                if state_abbr:
                ...
                log(logger, 'info', f"Looking for: {values_to_try[:3]}", 'ADDRESS_FILL', 'DOM')
                ...
                for option in options:
                    ...
                    for try_value in values_to_try:
                        ...
                        if (text.upper() == try_value.upper() or value.upper() == try_value.upper() or
                            ...
                            matched_text = text
                            log(logger, 'info', f"Matched '{try_value}': text='{text}', value='{value}'", 'ADDRESS_FILL', 'DOM')
                            break
                        
                        # Partial match
                        if try_norm in text_norm or try_norm in value_norm:
