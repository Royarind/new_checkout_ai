...
5. Decide whether to terminate the plan or not based on -
    ...
    3. If the same tool fails 3 times in a row, terminate with error.
    4. If you see random unrelated tools being called (web_search, screenshot when trying to fill forms), terminate.
    5. If 10+ iterations without progress, terminate.
    6. Common recovery: suggest using low-level actions (click, fill_text) instead of high-level tools.
    </deciding_termination>
...
CA_model = OpenAIModel(model_name = os.getenv("AGENTIC_BROWSER_TEXT_MODEL", "gpt-4o"))
CA_agent = Agent(
    model=CA_model, 
    name="Critique Agent",
