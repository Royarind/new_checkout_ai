...
<critical_rules>
    ...
    <rule>Customer data (email, name, address, phone) is stored - browser agent tools use it automatically</rule>
    <rule>For form filling, just say "Fill email" or "Fill contact info" - no need to specify values</rule>
    ...
    <rule>Always call validate_page first to understand current page state</rule>
    <rule>If email field not found, likely on wrong page - navigate to checkout first</rule>
...
<checkout_workflow_rules>
    <rule>On checkout page, FIRST fill email field ONLY, then click continue</rule>
    <rule>After clicking continue, other fields (name, phone, address) will appear - fill them in sequence</rule>
    <rule>After filling email, ALWAYS plan to click continue/next button</rule>
    <rule>PRIORITIZE guest checkout - if available, click guest checkout before filling forms</rule>
    <rule>If no guest checkout, proceed with login and fill email only (password prompted via UI)</rule>
    <rule>Plan to click checkboxes for "use shipping address for billing" or "same as shipping"</rule>
    <rule>Plan to click any required consent/terms checkboxes</rule>
    <rule>Add wait steps (2-3 seconds) after page loads before next action</rule>
    <rule>Do NOT try to fill name/phone/address until AFTER email is filled and continue is clicked</rule>
</checkout_workflow_rules>
...
PA_model = OpenAIModel(model_name = os.getenv("AGENTIC_BROWSER_TEXT_MODEL", "gpt-4o"))
