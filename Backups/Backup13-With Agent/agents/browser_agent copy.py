...
Customer data (email, name, address) is stored and automatically used by form-filling tools.
</agent_role>
...
- click_Continue() - Click continue/next button
- fill_contact() - Fill contact info (uses stored customer data)
- fill_address() - Fill address (uses stored customer data). Use auto-fill when possible.
- click_checkout() - Click checkout button
- click_guest_checkout() preferred - Click guest checkout else fill email and prompt for password
- click_continue() - Click continue/next
- dismiss_popups() - Close popups
- take_screenshot(path) - Capture screen
- validate_page() - Get page state
- check tick box for use same address for billing if prompted
- click_continue() after filling each section
- wait a few seconds for page loads
- successfully complete checkout flow up to payment page
