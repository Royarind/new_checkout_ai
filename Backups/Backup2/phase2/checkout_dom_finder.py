            ...
            (args) => {
                ...
                for (const keyword of keywords) {
                    ...
                    for (const field of fields) {
                        ...
                        // Priority 2: Name attribute (exact or partial match)
                        if (field.name) {
                            const normName = normalize(field.name);
                            if (normName.includes(normKeyword) || normKeyword.includes(normName)) {
                                field.setAttribute('data-checkout-marker', marker);
                                return { found: true, marker: marker, method: 'name' };
                            }
                        }
                        
                        // Priority 3: ID attribute (exact or partial match)
                        if (field.id) {
                            const normId = normalize(field.id);
                            if (normId.includes(normKeyword) || normKeyword.includes(normId)) {
                                field.setAttribute('data-checkout-marker', marker);
                                return { found: true, marker: marker, method: 'id' };
                            }
                        }
...
async def fill_input_field(page, label_keywords, value, max_retries=3):
    ...
    for attempt in range(max_retries):
        try:
            ...
            logger.error(f"CHECKOUT DOM: Error filling (attempt {attempt + 1}): {e}")
            if attempt < max_retries - 1:
                await asyncio.sleep(2)
            else:
