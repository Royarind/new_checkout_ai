    ...
    try:
        ...
        result = await page.evaluate("""
            () => {
                ...
                // Try each selector and click ALL matching elements
                    ...
                    try {
                        ...
                        for (const el of elements) {
                            ...
                            if (el.offsetParent !== null && typeof el.click === 'function') {
                                // Additional check: make sure it's not a primary action button
                                const text = el.textContent?.toLowerCase() || '';
                                const ariaLabel = el.getAttribute('aria-label')?.toLowerCase() || '';
                                
                                // Skip buttons that look like primary actions
                                const skipWords = ['add to cart', 'buy now', 'checkout', 'submit', 'subscribe', 'sign up'];
