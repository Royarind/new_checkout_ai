...
async def fill_contact_info(page, contact_data, user_prompt_callback=None, llm_client=None):
                ...
                if mapping and len(mapping) > 0:
                    ...
                    if result['filled_count'] >= 2:
                        ...
                        if continue_result['success']:
                            await asyncio.sleep(3)
                            # Check if more fields appeared after continue
                            from phase2.checkout_dom_finder import get_all_form_fields
                            new_fields = await get_all_form_fields(page)
                            if len(new_fields) == 0:
                                return {'success': True}
                            # Continue to fill remaining fields below
            except Exception as e:
    ...
    if contact_data.get('email'):
        ...
        continue_result = await find_and_click_button(page, CONTINUE_BUTTONS, max_retries=1)
        if continue_result['success']:
            logger.info(f"CHECKOUT FLOW: Continue clicked after email, waiting for next page...")
            await asyncio.sleep(3)
            # Re-check for new fields - wait for page to fully load
            await page.wait_for_load_state('networkidle', timeout=5000)
            from phase2.checkout_dom_finder import get_all_form_fields
            new_fields = await get_all_form_fields(page)
            logger.info(f"CHECKOUT FLOW: After continue, found {len(new_fields)} fields on page")
            for field in new_fields:
                logger.info(f"  Field: {field.get('name', 'N/A')} | {field.get('id', 'N/A')} | {field.get('type', 'N/A')}")
            
            # If LLM available, try to fill all remaining fields at once
            if llm_client and len(new_fields) > 0:
                try:
                    # Combine contact and address data
                    all_data = {**contact_data}
                    mapping = await _llm_map_contact_fields(llm_client, new_fields, all_data)
                    if mapping and len(mapping) > 0:
                        from phase2.checkout_dom_finder import batch_fill_fields
                        result = await batch_fill_fields(page, mapping)
                        logger.info(f"CHECKOUT FLOW: Batch filled {result['filled_count']} fields after continue")
                        filled_count += result['filled_count']
                        # Don't return yet - continue to try individual fields too
                except Exception as e:
                    logger.warning(f"CHECKOUT FLOW: Batch fill after continue failed: {e}")
    ...
    if contact_data.get('firstName'):
        ...
        if result['success']:
            ...
            log(logger, 'warning', 'First name field not found', 'ADDRESS_FILL', 'RULE_BASED')
    ...
    if contact_data.get('lastName'):
        ...
        if result['success']:
            ...
            log(logger, 'warning', 'Last name field not found', 'ADDRESS_FILL', 'RULE_BASED')
    ...
    if contact_data.get('phone'):
        ...
        if result['success']:
            ...
            log(logger, 'warning', 'Phone field not found (may be optional)', 'ADDRESS_FILL', 'RULE_BASED')
    
    # If we filled at least email, consider it success
    if filled_count >= 1:
        log(logger, 'info', f"Contact info completed ({filled_count} fields)", 'ADDRESS_FILL', 'RULE_BASED')
        return {'success': True}
    else:
        log(logger, 'error', 'Failed to fill any contact fields', 'ADDRESS_FILL', 'RULE_BASED')
        return {'success': False, 'errors': ['No contact fields filled']}
...
async def run_checkout_flow(page, customer_data, agent_coordinator=None, user_prompt_callback=None, use_ai_flow=True):
    ...
    try:
        ...
        # Notify UI that payment page is reached
        try:
            notification_file = '/tmp/chkout_payment_ready.txt'
            with open(notification_file, 'w') as f:
                f.write('PAYMENT_READY')
            logger.info(f"CHECKOUT FLOW: [{datetime.now().strftime('%H:%M:%S')}] Payment page reached - notification sent")
        except Exception as e:
            logger.warning(f"CHECKOUT FLOW: [{datetime.now().strftime('%H:%M:%S')}] Failed to write notification: {e}")
