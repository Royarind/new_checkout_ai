...
async def run_agentic_flow(page: Page, task: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # Construct initial query from task
    query = f"Navigate to {task.get('url')} and select variant {task.get('selectedVariant')} then add to cart with quantity {task.get('quantity')}. Finally proceed to checkout."
    if 'customer_data' in task:
        query += f" Use customer data: {task['customer_data']}"
    ...
    for i in range(max_iterations):
        ...
        # 1. Planner Agent
        logger.info("ORCHESTRATOR: Calling Planner Agent...")
        pa_input = {
            "query": query,
        ...
        logger.info(f"ORCHESTRATOR: Planner Next Step: {next_step}")
        ...
        tool_response = ba_result.output
        logger.info(f"ORCHESTRATOR: Browser Action Result: {tool_response}")
        
        # 3. Critique Agent
        logger.info("ORCHESTRATOR: Calling Critique Agent...")
        ca_input = str(CritiqueInput(
            current_step=next_step,
            orignal_plan=current_plan,
            tool_response=str(tool_response),
            ss_analysis="No screenshot analysis available yet" 
        ))
        ca_result = await CA_agent.run(ca_input)
        critique_data = ca_result.output
        
        feedback = critique_data.feedback
        logger.info(f"ORCHESTRATOR: Critique Feedback: {feedback}")
        
        if critique_data.terminate:
            logger.info("ORCHESTRATOR: Critique decided to terminate.")
            return {
                'success': True, 
...
async def run_full_flow(json_input):
    ...
    try:
        ...
        logger.info(f"ORCHESTRATOR: Launching Chrome with persistent profile at {profile_path}")
